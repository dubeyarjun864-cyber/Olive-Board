...
def generate_premium_html(content, exam_name):
    ...
    <head>
        ...
        <style>
            ...
            .header {{ background: #007bff; color: white; padding: 15px; display: flex; justify-content: space-between; position: sticky; top: 0; z-index: 100; }}
            .q-card {{ background: white; margin: 15px; padding: 20px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }}
            .sol-btn {{ background: #28a745; color: white; border: none; width: 100%; padding: 12px; border-radius: 8px; cursor: pointer; font-weight: bold; }}
            .solution {{ display: none; background: #f8f9fa; padding: 15px; border-left: 5px solid #28a745; margin-top: 10px; }}
            .footer {{ position: fixed; bottom: 0; width: 100%; background: white; text-align: center; padding: 15px; border-top: 1px solid #ddd; }}
    ...
    <body>
        <div class="header">
            ...
            <button onclick="toggleLang()" style="border-radius:20px; border:none; padding:5px 15px; cursor:pointer;">HI / EN</button>
        </div>
        <div class="container">{content}</div>
        <div class="footer">Developed by <a href="{CHANNEL_LINK}">ArjunBotz</a> | Extracted by <a href="{ADMIN_LINK}">Arjun Dubey</a></div>
        <script>
            function toggleLang() {{
...
def run_extraction(email, password, url, exam):
    ...
    try:
        ...
        # Real scraping data logic placeholder
